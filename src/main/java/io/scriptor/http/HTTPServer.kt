...
import io.scriptor.log.warning
...
class HTTPServer(
    ...
    private fun <S, D> convert(obj: S, source: Type, destination: Type): D {
        ...
        if (converters.containsKey(source) && converters[source]!!.containsKey(destination)) {
            ...
            socket.getOutputStream().use { outputStream ->
                ...
                try {
                    for (i in 0..<parameterCount) {
                        ...
                        } else if (parameter.isAnnotationPresent(io.scriptor.annotation.Parameter::class.java)) {
                            val name = parameter.getAnnotation(io.scriptor.annotation.Parameter::class.java).value
                    ...
                    result = convert(obj, type, object : TypeRef<HTTPResult<*>>() {}.type)
                } catch (e: Exception) {
                    trace(e)
                    HTTPResponseMessage(
                        "HTTP/1.1",
