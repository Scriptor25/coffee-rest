    ...
    static boolean isAssignable(final @NotNull Type dst, final @NotNull Type src) {
        ...
        return switch (dst) {
            case Class<?> dc -> switch (src) {
                ...
                case GenericArrayType sa -> dc.isArray()
                                            && isAssignable(dc.getComponentType(), sa.getGenericComponentType());
            ...
            case ParameterizedType dp -> switch (src) {
                ...
                case GenericArrayType sa -> dp.getRawType() instanceof Class<?> toRawC
                                            && toRawC.isArray()
                                            && isAssignable(toRawC.getComponentType(), sa.getGenericComponentType());
                case TypeVariable<?> sv -> isInBounds(dst, sv.getBounds(), null);
                case WildcardType sw -> isInBounds(dst, sw.getUpperBounds(), sw.getLowerBounds());
                default -> false;
            };
            case GenericArrayType da -> switch (src) {
                case Class<?> sc -> sc.isArray()
                                    && isAssignable(da.getGenericComponentType(), sc.getComponentType());
                case ParameterizedType sp -> sp.getRawType() instanceof Class<?> fromRawC
                                             && fromRawC.isArray()
                                             && isAssignable(da.getGenericComponentType(), fromRawC.getComponentType());
